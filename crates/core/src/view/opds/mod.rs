...
impl Opds {
    pub fn new(
        ...
        if context.settings.opds.is_empty() {
            bail!("No OPDS Servers in settings");
        }
        let selected_server = 0;
        let opds_settings = &context.settings.opds[0];
        let fetcher = OpdsFetcher::new(opds_settings.clone())?;
    ...
    fn update_feed(
        ...
        if feed.is_ok() {
            self.feed = feed.unwrap();
            self.update_view(hub, rq, context);
        } else {
            hub.send(Event::Notify("Failed to download feed".to_owned()))
                .ok();
            println!("Failed to download feed: {:?}", feed.err())
        }
    }
    ...
    fn update_bottom_bar(&mut self, _hub: &Hub, rq: &mut RenderQueue, _context: &Context) {
        if let Some(index) = rlocate::<BottomBar>(self) {
            let bottom_bar = self.children[index]
                ...
                .unwrap();
            bottom_bar.update_server_label(&self.fetcher.settings.name, rq);
    ...
    fn load_server(
        ...
        if fetcher.is_ok() {
            ...
            self.fetcher = fetcher.unwrap();
            let feed = self.fetcher.home();
            self.update_feed(feed, hub, rq, context);
        } else {
            hub.send(Event::Notify("Failed to switch server".to_owned()))
                .ok();
            println!("Failed to switch server: {:?}", fetcher.err())
        }
    }
        ...
        if let Some(entry) = self.feed.entries.iter().find(|x| x.id == *entry_id) {
            ...
            let download_link = entry.links.iter().find(|&x| {
                (x.mime_type == Some(MimeType::Epub)
                    ...
                    && x.href.is_some() && x.mime_type.is_some()
            });
            if let Some(link) = download_link.clone() {
                ...
                let save_path = fetcher.settings.download_location.clone();
                ...
                thread::spawn(move || {
                    hub2.send(Event::Notify(format!("Downloading: {}", title))).ok();
                    
                    let file = fetcher.download_relative(&href, &file_path);
                    if file.is_ok() {
                        hub2.send(Event::Notify(format!("Downloaded: {}", title))).ok();
                        hub2.send(Event::OpdsDocumentDownloaded).ok();
                    } else {
                        hub2.send(Event::Notify("Failed download file".to_owned()))
                            .ok();
                        println!("Failed download file: {:?}", file.err())
                    }
                });
                return;
            }
            ...
            if let Some(link) = navigation_link {
                ...
                let feed = self.fetcher.pull_relative(&href);
                self.update_feed(feed, hub, rq, context);
                return;
            }
        }
